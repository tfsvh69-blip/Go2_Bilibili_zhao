...
PLATFORM_ROOT = PACKAGE_ROOT.parents[1] / "platform"
...
def test_forward_mppi_is_bounded_diff_drive_comparison_profile():
    ...
    # 前向约束只属于自主 MPPI；共享 smoother 必须保留键盘安全倒车，
    # 且不得超过 Go2 CHAMP gait.yaml 的 0.30 m/s、0.50 rad/s 上限。
    smoother = _parameters(
        "controller_forward_mppi.yaml", "velocity_smoother")
    assert smoother["max_velocity"] == [0.30, 0.0, 0.50]
    assert smoother["min_velocity"] == [-0.30, 0.0, -0.50]
...
def test_navigation_uses_planar_frames_scan_sources_and_correct_depth_topic():
    ...
    assert params["general_goal_checker"]["yaw_goal_tolerance"] == 0.10
...
def test_goal_guard_tolerates_only_shutdown_runtime_error():
    source = (PACKAGE_ROOT / "go2_navigation" / "goal_guard.py").read_text(
        encoding="utf-8")
