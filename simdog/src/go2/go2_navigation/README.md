...
python3 simdog/src/go2/go2_navigation/tools/nav_tuner.py
```
...
Global Costmap 当前按用户现场值使用 `inflation_radius=0.50 m`、
`cost_scaling_factor=0.5`；Local Costmap 仍保持 `0.30 m/3.0`。较宽全局半径会扩大
障碍周围的代价带，较低衰减因子会让这 0.50 m 范围内的代价下降更慢；它会让路径更早
远离墙面，也可能封住窄通道，但不删除错误 `/scan` 端点，也不等于完成安全标定。阶段 3
...
共享 `velocity_smoother` 则按 CHAMP gait 上限使用
`max_velocity=[0.30,0.0,0.50]`、`min_velocity=[-0.30,0.0,-0.50]`：这只让键盘和
Unitree Move 能够安全倒车，不会改变 MPPI 的 `vx_min=0.0` 前向采样约束。
...
`1.0 rad/s²` 对齐到 `0.10 rad` yaw 容差；shim 使用限加速度开环命令，路径进入/退出
