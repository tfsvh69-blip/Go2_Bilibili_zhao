...
因此项目正式默认值现为 `+0.20..+0.30 m`。这条结论是用户现场 A/B 结果；完整走遍场景、
保存地图并用固定图导航的端到端验收仍要按下文完成，不能提前写成全部导航安全门通过。
历史原始 CSV/JSON 位于
...
正式 `/scan` 的现场有效高度层为 `+0.20..+0.30 m`；调试 `/scan_raw` 固定保留旧
...
固化后已在隔离无界面 Gazebo 读回正式/原始窗口 `0.20..0.30` 与
...
当前项目默认值为 `0.20/0.30 m`，正式建图时保持这两个值。只有问题复现时才用 rqt
做临时 A/B。始终满足 `min_height < max_height`；恢复当前默认值时先把
`max_height` 设为 `0.30`，再把 `min_height` 设为 `0.20`。非法值会被节点拒绝，终端会
显示原因。
...
```bash
ros2 param get /go2_lidar_scan_converter min_height   # 期望 0.2
ros2 param get /go2_lidar_scan_converter max_height   # 期望 0.3
...
应看到上述五类文件，`session.yaml` 中应记录 `lidar_min_height_m: 0.2`、
`lidar_max_height_m: 0.3`。
