...
def test_converter_uses_valid_infinity_contract():
    ...
    assert params["min_height"] == 0.20
    assert params["max_height"] == 0.30
    ...
    assert diagnostics["expected_min_height"] == 0.20
    assert diagnostics["expected_max_height"] == 0.30
...
def test_converter_really_updates_height_and_rejects_fake_runtime_changes():
    source = (
        PACKAGE_ROOT.parent / "pointcloud_to_laserscan" / "src" /
        "pointcloud_to_laserscan_node.cpp"
    ).read_text(encoding="utf-8")
    nav_launch = (
        PACKAGE_ROOT.parent / "go2_navigation" / "launch" /
...
def test_nav2_costmap_matches_scan_clearing_contract():
    navigation = yaml.safe_load(
        (PACKAGE_ROOT.parent / "go2_navigation" / "config" /
...
def test_gazebo_vlp16_uses_motion_realtime_sampling():
    xacro_path = (
        PACKAGE_ROOT.parent / "unitree-go2-ros2" / "robots" / "descriptions" /
...
def test_d435_render_load_matches_navigation_profile():
    xacro_path = (
        PACKAGE_ROOT.parent / "realsense_ros_gazebo" / "xacro" /
        "depthcam.xacro"
    )
    text = xacro_path.read_text(encoding="utf-8")
...
def test_navigation_ground_truth_does_not_inject_pose_noise():
    gazebo_path = (
        PACKAGE_ROOT.parent / "unitree-go2-ros2" / "robots" /
