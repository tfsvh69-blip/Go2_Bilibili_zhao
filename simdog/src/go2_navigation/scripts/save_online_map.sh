...
echo "[1/4] 保存 Slam Toolbox pose graph"
...
read_ros_parameter() {
    ...
    local value
    if value="$(ros2 param get --hide-type "${node_name}" "${parameter_name}" 2>/dev/null)"; then
        printf '%s' "${value}"
    else
        printf '%s' "unknown"
    fi
...
echo "[3/4] 写入会话索引与雷达参数"
saved_at="$(date --iso-8601=seconds)"
lidar_min_height="$(read_ros_parameter /go2_lidar_scan_converter min_height)"
lidar_max_height="$(read_ros_parameter /go2_lidar_scan_converter max_height)"
lidar_range_min="$(read_ros_parameter /go2_lidar_scan_converter range_min)"
lidar_range_max="$(read_ros_parameter /go2_lidar_scan_converter range_max)"
{
    printf "format: go2_online_map_session_v1\n"
    printf "map_name: '%s'\n" "${map_name}"
    printf "saved_at: '%s'\n" "${saved_at}"
    printf "map_source: slam_toolbox\n"
    printf "scan_topic: /scan\n"
    printf "scan_frame: velodyne_level\n"
    printf "lidar_min_height_m: %s\n" "${lidar_min_height}"
    printf "lidar_max_height_m: %s\n" "${lidar_max_height}"
    printf "lidar_range_min_m: %s\n" "${lidar_range_min}"
    printf "lidar_range_max_m: %s\n" "${lidar_range_max}"
    printf "files:\n"
    printf "  occupancy_yaml: map.yaml\n"
    printf "  occupancy_image: map.pgm\n"
    printf "  posegraph: slam.posegraph\n"
    printf "  posegraph_data: slam.data\n"
} > "${temp_dir}/session.yaml"

if [[ ! -s ${temp_dir}/session.yaml ]]; then
    echo "在线地图保存结果缺少：session.yaml" >&2
    exit 1
fi
...
echo "会话索引：${final_dir}/session.yaml"
echo "下次续建："
